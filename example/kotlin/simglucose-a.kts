...
val bgValues = listOf(55.0, 70.0, 240.0) // <300?
...
val stlList = listOf(
    //"!((input(0) == 50.0 -> X (input(0) == 0.0)) U (! ($max_bg < 240.0)))",
    "(input(0) == 50.0 && X (input(0) == 50.0)) R ($max_bg < 240.0)",
    "(input(0) == 50.0 && X (input(0) == 50.0)) R ($bg < 70.0 -> X ($max_bg > 70.0))", //下位10%以下を 30 分以上取らない
    "(<> (input(0) == 50.0 && X (input(0) == 50.0))) R ! []_[0,5] ($max_bg < 70.0)", //低血糖状態が150分以上続かない
...
PythonContinuousNumericSUL(initScript, signalStep).use { autoTransSUL ->
    ...
    if (result) {
        ...
        for (i in 0 until verifier.cexProperty.size) {
            ...
            /*
            val concreteOutput = ArrayList<ArrayList<Double>>()
            for (j in 0 until verifier.cexOutput[i].size()) { //error
                val e = verifier.cexOutput[i].getSymbol(j)
                val ls = e.toCharArray()
                val ary = ArrayList<Double>()
                for (k in 0 until ls.size) {
                    ary.add(outputMapperReader.outputMapper[k].getOrDefault(ls[k], -1.0))
                }
                concreteOutput.add(ary)
            }
            */
        }
    }
